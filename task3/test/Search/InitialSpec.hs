...
spec = do
  describe "Initial#findStartingPlayer1()" $ do
    it "succeeds" $ do
      True `shouldBe` True
      isValid (findStartingPlayer1 0 0 Nothing) `shouldBe` True
      isValid (findStartingPlayer1 1 0 Nothing) `shouldBe` True

  describe "Initial#findStartingPlayer2()" $ do
    it "succeeds" $ do
      let p1 = [((x `div` 5 + 1, x `mod` 5 + 1), (y `div` 5 + 1, y `mod` 5 + 1)) | x <- [0 .. 24], y <- [0 .. 24], x /= y]
      all (\p -> isValid $ findStartingPlayer2 0 0 p Nothing) p1 `shouldBe` True
      all (\p -> isValid $ findStartingPlayer2 1 0 p Nothing) p1 `shouldBe` True
